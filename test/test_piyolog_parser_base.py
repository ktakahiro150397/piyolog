...
def test_parse_line_memo():
    ...
    # Arrange
    test_line = "17:20   メモ   うんちが出る雰囲気がない🤔"
    parser = PiyoLogParserBase()
    ...
    # Assert
    assert record.date == datetime(2024, 12, 1, 17, 20, 0)
    assert record.record_type == "メモ"
    assert record.additional_record_data == ""
    assert record.record_memo == "うんちが出る雰囲気がない🤔"
...
def test_parse_line_temperature():
    ...
    # Assert
    assert record.date == datetime(2024, 12, 1, 13, 45, 0)
    assert record.record_type == "体温"
    assert record.additional_record_data == "36.5°C"
    assert record.record_memo == ""
